...
# Длинная сторона кадра для языковой модели. Оставлено 640: это ручка «качество против
# скорости», и выбор за продуктом, а не за замером. Сами числа на 87 шагах
# assembly101-fine (винтовая группа — те 23 шага, где надо различить закручивание и
# откручивание, и где мелкая моторика тонет в двух процентах площади кадра):
#   640 px   винтовая группа  6/23, предмет 13/87, 2.64 с на шаг
#   960 px   винтовая группа 11/23, предмет 16/87, 3.48 с на шаг  (+32 % времени)
#   1280 px  винтовая группа  9/23, предмет 16/87, 4.45 с на шаг  (+68 %)
# То есть у разрешения есть оптимум и он не на краю: при 1280 кадр даёт 720 зрительных
# токенов против 510 на 960, и на длинном шаге из 24 кадров это 17 тысяч токенов против
# двенадцати — промпт с инструкцией тонет в картинках, тот же механизм вытеснения, что и
# при лишних кадрах. Включается одной переменной, когда решим, что важнее.
...
# Отдавать модели измеренные признаки кисти текстом: сторона, раскрытие в начале и в конце
# шага, путь запястья. Замер на 87 шагах assembly101-fine: кисть закрывается на девяти
# шагах «взял» из одиннадцати, то есть признак несёт направление времени — самый крупный
# класс наших ошибок после обобщения. На «положил» он слабее (раскрывается 11 из 20),
# поэтому подаётся как подсказка, а не как правило.
HANDS_NOTE = os.getenv("PRAXIS_HANDS_NOTE", "0").lower() not in {"0", "false", "no"}
