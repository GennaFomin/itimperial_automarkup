...
**`changepoint-review-truong2020.pdf`** — Truong, Oudre, Vayatis,
*Selective review of offline change point detection methods*, Signal Processing 2020.
[arXiv:1801.00718](https://arxiv.org/abs/1801.00718)

This is the paper behind our boundary algorithm. Our `tsm-kernel` segmenter is penalised
kernel change-point detection solved by exact dynamic programming: a per-segment penalty
selects the number of cuts, and the kernel cost is computed over the self-similarity
matrix of video features. Sections on the kernel cost function and on penalty-based model
selection are the ones that matter here.
...
**paywalled, not attached.** This applies exactly the above method to video and is the
closest published validation of our approach: on EPIC-KITCHENS-55 their unsupervised
segmentation reaches f1@50 21.4 against 10.7 for a trained method.
